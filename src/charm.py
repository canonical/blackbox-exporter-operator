...
from constants import PROBES_RELATION_NAME, PEERS_RELATION_NAME
...
class BlackboxExporterOperatorCharm(ops.CharmBase):
    ...
    def _update_peer_relation_data(self):
        if not self.model.get_relation(PEERS_RELATION_NAME):
            return
        self.unit.status = MaintenanceStatus(f"Updating peer relation data")
        peer_relation_data = {
        ...
        logger.info(peer_relation_data)
